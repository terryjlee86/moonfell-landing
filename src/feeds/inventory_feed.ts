...
let _items: InvItem[] = [
  ...
  // Wearables (example: start with nothing equipped)
  ...
  // Example not-equipped items in pack:
  // { id: "leather-cap", name: "Leather Cap", kind: "wearable", slot: "head", equipped: false, armorTier: "light" },
];
...
export function inventoryFeed(): { tags: string[]; list: { items: InvItem[] } } {
  ...
  // Umbrella ranged tags (for legacy/generic flows)
  ...
  // Emit wear tags for EQUIPPED items, and pack tags for NOT equipped items.
  // Also compute aggregate armor tier (max of equipped tiers: heavy > medium > light).
  let armorRank = 0; // 0 none, 1 light, 2 medium, 3 heavy

  const rank = (t?: ArmorTier) => (t === "heavy" ? 3 : t === "medium" ? 2 : t === "light" ? 1 : 0);

  items.forEach((it) => {
    if (it.kind !== "wearable" || !it.slot) return;

    const slot = it.slot; // guaranteed WearableSlot
    const slug = slugify(it.name || it.id);

    const isEquipped = Boolean(it.equipped);
    if (isEquipped) {
      // Presence tag and specific item tag
      tags.push(`pc:wear:${slot}`);
      tags.push(`pc:wear:${slot}:${slug}`);

      armorRank = Math.max(armorRank, rank(it.armorTier));
    } else {
      // In-pack variant tag (not equipped but carried)
